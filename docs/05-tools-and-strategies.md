...
- **What is lost if Redis is lost.** No data: cached reads rebuild from PostgreSQL and the API logs one warning and continues. What is delayed: the rotation worker's schedule lives in Redis, so a draw due during an outage runs on the next tick after Redis returns, or immediately via `pnpm scheduler --once`. When the camera queue exists, events in flight would be lost, so that queue will need Redis persistence or a durable broker. That choice is deferred to the license plate work ([ADR-0009](adr/0009-lpr-as-async-event-subsystem.md)).
