...
| Vitest | Same runner for pure domain tests, PostgreSQL-backed integration tests (against a dedicated `parking_test` database that the global setup creates and migrates), and React component tests (Testing Library on jsdom). |
...
**Data protection.**
- TLS at the load balancer; in the reference topology the API tasks sit in private subnets and only the load balancer is reachable from the internet (the process itself binds the container's interface; exposure is a deployment property).
...
- No personal data in application logs. The request logger records a fixed field set (request id, method, route, path, status, duration); bodies, query strings, and cookies are never logged, and this API's paths carry route names and UUIDs only. Every logged error, in the HTTP handler, the rotation worker, and the cache client, passes through `describeError`, which keeps the SQL text with placeholders, the SQLSTATE, and the constraint name but drops bound parameters, row values, and the values PostgreSQL quotes in its own messages; tests feed it errors carrying an email in each of those places and assert the email is gone. Reviewed against the pull request checklist.

**Authentication and authorization** ([ADR-0008](adr/0008-mock-auth-jwt-cookie-rbac.md)).
- Session is an HS256 JWT with a 12 hour expiry in an `HttpOnly`, `SameSite=Strict` cookie, flagged `Secure` in production (local development runs over plain HTTP, where a `Secure` cookie would never be sent back). Scripts cannot read it; cross-site requests do not carry it.
...
- Least-privilege task roles: the API and the rotation worker reach RDS and ElastiCache, the one-off migration task reaches RDS only, and nothing else.
- Database and cache in private subnets; security groups admit exactly those three task roles on the port each needs, matching the deployment diagram in [01-architecture.md](01-architecture.md).
...
- **Logs.** `pino` JSON to stdout, shipped by the platform. One line per request with the request id (`X-Request-Id`, honored from nginx or the load balancer, generated otherwise), method, matched route, status, and duration. Bodies, query strings, and cookies are never logged; paths are, and in this API they carry route names and UUIDs only. Every logged error passes through `describeError` (see the security section); unhandled HTTP errors also carry the request id.
...
- **Decision record.** [ADR-0013](adr/0013-observability-with-pino-and-prometheus.md) covers the choice of pino and prom-client over OpenTelemetry or a vendor agent; [ADR-0014](adr/0014-single-file-container-images.md) covers the image strategy.
...
- **What is lost if Redis is lost.** No data: cached reads rebuild from PostgreSQL and the API logs one warning and continues. What is delayed: the rotation worker's schedule lives in Redis. If Redis restarts with its data, the schedule survives and a draw due during the outage runs on the next tick; if Redis is wiped, the worker re-registers the schedule the next time it starts, and `pnpm scheduler --once` runs the check immediately from any host. When the camera queue exists, events in flight would be lost, so that queue will need Redis persistence or a durable broker. That choice is deferred to the license plate work ([ADR-0009](adr/0009-lpr-as-async-event-subsystem.md)).
