...
- Sessions are signed JWTs carrying user id, role, and building id, stored in a cookie flagged `HttpOnly`, `Secure`, and `SameSite=Strict`. Scripts cannot read the token, and cross-site requests do not carry it.
