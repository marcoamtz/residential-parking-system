...
| --- | --- | --- | --- | --- |
| Domain engine (`packages/domain`) | Me | Nothing | Pure `executeDraw`; unit tests for every ranking rule, tie-break determinism, and input validation | Done |
| Schema and migrations (`packages/db`) | Me | Nothing | Every uniqueness invariant and the entrant-to-cycle ownership are constraints (composite foreign key); same-building ownership of registrations and spots is enforced by the building-scoped services until Row-Level Security; registration closure is a row lock shared with the draw; migration applies to a fresh database; migrator script | Done |
| Draw transaction (`apps/api/src/admin/draw.ts`) | Me | Domain, schema | Single transaction, state-transition mutex, verification record; integration tests for replay, 409, concurrency, tenant scoping | Done |
